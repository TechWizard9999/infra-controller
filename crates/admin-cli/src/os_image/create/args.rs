...
pub(crate) struct Args {
    ...
    #[clap(
        short = 'b',
        long,
        help = "Whole-disk target that the image overwrites. Accepts 'smallest', /dev/nvme<controller>n<namespace>, /dev/sd<letters>, /dev/vd<letters>, or /dev/disk/by-id/<identifier>. If omitted or empty, selection prefers a disk with an EFI partition, then the first deterministically ordered whole disk."
    )]
